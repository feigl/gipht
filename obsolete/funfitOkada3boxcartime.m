function varargout = funfitOkada3boxcartime(varargin)
...
% 2017-SEP-19 add second reference epoch for boxcar time function
...
if (nin == 1 || nin == 2) && nout == 2 && isstruct(varargin{1}) == 0
    ...
    j = mp;
    j=j+1;pnames{j} = sprintf('Boxcar Start time in years     '); pscl(j)=1.0;
    j=j+1;pnames{j} = sprintf('Boxcar Stop  time in years     '); pscl(j)=1.0;
...
%     fprintf(1,'Unpacking parameter vector\n');
    ...
    kp=kp+1;F.z = pg(kp);
    %     j=j+1;pnames{j} = sprintf('Okada3 dX in m                 '); pscl(j)=1.0E-3;
    % all 3 dimensions must be equal to each other in a cube
    kp=kp+1;F.dx = pg(kp);
    F.dy = pg(kp);
    F.dz = pg(kp);
...
%            uokada = okada85_wrapper3(rowvec(vecx),rowvec(vecy),F(i),nu); % use Kurt's function
           uokada = okada85_wrapper3(rowvec(DST.x),rowvec(DST.y),F,nu); % use Kurt's function
...
%     disp 'u'; size u
    ...
    %tdif = DD * timefun(pt, tstart);
    if strcmp(rowvec(PST.timefun),'timefun_boxcar') == 1 && tstart < tstop
        tdif = DD * timefun(pt, tstart, tstop);
    else
        tstart
        tstop
        error('Incorrect time function. Set timefun = timefun_boxcar\n');
    end
    fprintf(1,'Time difference in years %10.4f\n',tdif);
