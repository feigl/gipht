function [uabaqus] = abaqus2d_mogi1(PST,xobs,yobs,TST)
...
%     Sxy_node = mphinterp(model,{'poro.sxy'},'coord',coords,'t',[PST.t1 PST.t2]); % xy component
    ...
    [upts,vpts,wpts] = interpolate2Ddisp(PST,xobs,yobs,rnode,znode,unode,wnode);
...
if PST.verbose == 1;
    % plot the results
    figure; hold on;
    quiver(xobs/1.e3,yobs/1.e3,upts,vpts);
    xlabel('Easting coordinate [km]');
    ylabel('Northing coordinate [km]');
    
    figure;hold on;
    iok = find(abs(yobs) <= 1.);
    plot(xobs(iok)/1.e3,upts(iok)*1.e3,'r+-');
    plot(xobs(iok)/1.e3,vpts(iok)*1.e3,'go-');
    plot(xobs(iok)/1.e3,wpts(iok)*1.e3,'b^-');
    plot(xobs(iok)/1.e3,mpts(iok)*1.e3,'k*-');
    xlabel('Easting coordinate [km]');
...
% %  dupts  = interp2(x_node,y_node,du_node,xobs,yobs,'cubic'); % easting component
% %  dvpts  = interp2(x_node,y_node,dv_node,xobs,yobs,'cubic'); % northing component
% %  dwpts  = interp2(x_node,y_node,dw_node,xobs,yobs,'cubic'); % vertical component
% 
% % evaluate functions at observation locations
...
uabaqus = zeros(3,numel(xobs));
uabaqus(1,:) = rowvec(dupts);
uabaqus(2,:) = rowvec(dvpts);
uabaqus(3,:) = rowvec(dwpts);
% size(uabaqus)
