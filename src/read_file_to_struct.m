function [values, name] = read_file_to_struct(filename,nheaders,ncols,iname, break_char)
...
% Modified by Elena C Reinisch 20170831 to return values as a structure
% with option for break characters in file
...
if nargin < 5
    break_char = '';
    warning('Assuming no break character!');
end
...
while count > 0
    % get one line
    s = fgetl(fid);
    % check to see if line is a break line
    if isempty(strfind(s,break_char)) ~= 1
        in = 0;
    ...
    elseif isstr(s)
        ...
        if (count == nvals)
            values.(sprintf('vals%d', page_count))(in,1:nvals) = dumarr';
        end;
    else
        count = 0;
    end;
