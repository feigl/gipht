...
% break the following separately into x and y
for jj=1:nedges
    i1=kedges(jj,1);
    i2=kedges(jj,2);
    % ion is whether it is on the border or not
    if ion(i1) == 0 && ion(i2) == 0
        ...
        DEL(jj,i1) = -1./dist;
        DEL(jj,i2) = +1./dist;
    elseif ion(i1) == 1 % is on border 
        DEL(jj,i1) = 1;
    elseif ion(i2) == 1 % currently has the effect of setting this element to zero
        DEL(jj,i1) = 1;
    end
end
DEL=sparse(DEL);
