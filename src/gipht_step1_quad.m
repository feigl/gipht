...
grx_all = zeros(size(demz));
gry_all = zeros(size(demz));
...
%    end
    ...
    %% find gradient
 pixinpatch  = 1        % mininum number of valid (nonzero) pixels in a patch
  ithresh     = 32 %64 %32       % minimum misfit (circular mean deviation) to mean ( 1 DN is 1 / 256 pixel)
 maxcmd      =  16 %32 %16      % minimum misfit (circular mean deviation) to ramp ( 1 DN is 1 / 256 pixel) 
   qlsnam = nan;
   pha2qlsnam = nan;
   qspnam = 'qsp.pha';
   grxnam = 'grx.i2';
   grynam = 'gry.i2';
   maxpix = 400;
   write_pha('obs.pha', grdd*128/pi)
   nquad1 = pha2qls('obs.pha',ncols,nrows,qspnam,grxnam,grynam,qlsnam,ithresh,pixinpatch,maxcmd,maxpix,pha2qlsnam);
...
maxabs=0.5;
   ...
   dt_pair =  dyear(year(t2(i)), month(t2(i)), day(t2(i))) - dyear(year(t1(i)), month(t1(i)), day(t1(i)));
   dt_pair   

  %% add gradients to stack
   grx_all = grx_all + grx;%./dt_pair;
   gry_all = gry_all + gry;%./dt_pair;

  % save phase files
   fpath = strsplit(char(pfnames(i)), '/');
   pair_name = strcat(fpath{4}, '_quad_', fpath{5});
   DEM.info  = grdinfo3(demdescfile)
   DEM.info.description = sprintf('wrapped phase after quadtree resampling');
   DEM.info.command = sprintf('pha2qls (gipht_quad)')
   DEM.info.zname = 'radians';
   grdwrite3(grdx, grdy, qsp, pair_name, DEM.info)
...
dt_pairs = dyear(year(t2(end)), month(t2(end)), day(t2(end))) - dyear(year(t1(1)), month(t1(1)), day(t1(1)));
grx_all = grx_all./dt_pairs;

%% same gradient grds
DEM.info  = grdinfo3(demdescfile)
DEM.info.description = sprintf('stack of easting phase gradient');
DEM.info.command = sprintf('pha2qls (gipht_quad)') 
DEM.info.zname = 'rad/(yr*m)'; 
%[demx,demy,demz] = grdread3('/t31/insar/ENVI/T27/sanem/mst_corr/gipht/SQUEESAR/gradx_all_utm.grd');
[demx,demy,demz] = grdread3(demdescfile);

grdwrite3(demx, demy, grx_all, 'gradx_all_utm.grd', DEM.info)
grdwrite3(demx, demy, grx_all/np, 'gradx_avg_utm.grd', DEM.info)

DEM.info.description = sprintf('stack of northing phase gradient');
grdwrite3(demx, demy, gry_all, 'grady_all_utm.grd', DEM.info)
grdwrite3(demx, demy, gry_all/np, 'grady_avg_utm.grd', DEM.info)
