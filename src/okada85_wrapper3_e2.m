    ...
    for i=1:3
        switch i
            case 1 % horizontal fault
                dipd    =    0;% F.dip+0;  % dip in degrees
                striked =   0; % F.strike+0;  % strike in degrees
            ...
            case 2 % vertical fault in X-Z plane
                ...
                striked =   90; %F.strike+90;  % strike in degrees clockwise from north
                length  = F.dx(j); % length of fault in meters
                width   = F.dz(j); % width of fault in meters
            case 3 % vertical fault in X-Y plane
                dipd    =   90;%F.dip+90;  % dip in degrees
                striked =    0; %F.strike+0;  % strike in degrees clockwise from north
...
%         open    = open    * onev;
        ...
        uE = uE.*cosd(F.strike).*cosd(F.dip);
        uN = uN.*sind(F.strike).*cosd(F.dip);
        uN = uN.*sind(F.dip);
