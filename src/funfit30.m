    ...
    switch idatatype1
        ...
        case -1 % observable is east gradient
            if abs(volgeom1(4)) > 0
                ...
                umogi1 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1)';
            else
                umogi1 = zeros(3,ndata);
            end
            if abs(volgeom2(4)) > 0
                ...
                umogi2 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1)';
            else
                umogi2 = zeros(3,ndata);
            end
        otherwise
...
%                 uokada1 = disloc(pg(kp:kp+9),[DST.x';DST.y'],nu);
        ...
        case -1  % observable is gradient
            ...
            if  sum(abs(pg(kp+7:kp+9))) > 1.e-3             
                ...
                uokada1 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
%                 size(uokada1)
%                 
%                 figure;
%                 hist(uokada1(3,:));
...
%                 fprintf(1,'extrema of uokada1 %g %g\n',nanmin(nanmin(uokada1)),nanmax(nanmax(uokada1)));
            ...
            if  sum(abs(pg(kp+7:kp+9))) > 1.e-3
                tl = okada85_wrapper(pg(kp:kp+9),[DST.x'-DST.dx';DST.y'],nu);
                tr = okada85_wrapper(pg(kp:kp+9),[DST.x'+DST.dx';DST.y'],nu);
                uokada2 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
            else
                uokada2 = zeros(3,ndata);
            end
            kp = 56;
            ...
            if  sum(abs(pg(kp+7:kp+9))) > 1.e-3
                ...
                tr      = okada85_wrapper(pg(kp:kp+9),[DST.x'+DST.dx';DST.y'],nu2 ) ...
                    -     okada85_wrapper(pg(kp:kp+9),[DST.x'+DST.dx';DST.y'],nu  );
                uokada3 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
            else
                uokada3 = zeros(3,ndata);
            end
        otherwise
    ...
    if (abs(pg(kp+3)) > 1)
        ...
        switch idatatype1
            ...
            case -1 % observable is gradient
                ...
                uYangPS(1,:) = (ruY1 - luY1)/2.0 ./ repmat(DST.dx',3,1);
                uYangPS(2,:) = (ruY2 - luY2)/2.0 ./ repmat(DST.dx',3,1);
                uYangPS(3,:) = (ruY3 - luY3)/2.0 ./ repmat(DST.dx',3,1);
            otherwise
                warning(sprintf('Unknown value of idatatype1 = %d\n',idatatype1));
    ...
    if rho_ld > 1.0
        switch idatatype1
            ...
            case -1 % observable is gradient
                ...
                upinel = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
            otherwise
                warning(sprintf('Unknown value of idatatype1 = %d\n',idatatype1));
        ...
        case {0,2}  % observable is phase or range
            ...
            case -1 % observable is gradient
                ...
                tr = penny(DST.x'+DST.dx',DST.y'...
                    ,pg(kp+0),pg(kp+1),nu,E ...
                    ,pg(kp+2),pg(kp+4),pg(kp+3));
                uSunDisk1 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
            otherwise
                warning(sprintf('Unknown value of idatatype1 = %d\n',idatatype1));
        ...
        switch idatatype1
            ...
            case -1 % observable is gradient
                ...
                tr = penny(DST.x'+DST.dx',DST.y'...
                    ,pg(kp+0),pg(kp+1),nu,E ...
                    ,pg(kp+2),pg(kp+4),pg(kp+3));
                uSunDisk2 = (tr - tl)/2.0 ./ repmat(DST.dx',3,1);
            otherwise
                warning(sprintf('Unknown value of idatatype1 = %d\n',idatatype1));
    ...
    if abs(pg(kp)) > 0. 
        ...
        switch idatatype1
            ...
            case -1 % observable is gradient
                % calculate radial velocity field
                Vrvl = radial_velocity(pg(kp:kp+5),DST.x-DST.dx,DST.y);
                Vrvr = radial_velocity(pg(kp:kp+5),DST.x+DST.dx,DST.y);
                Vrv = (Vrvr - Vrvl) / 2.0 ./ repmat(DST.dx',3,1);
                % convert velocities to displacements D Is R * T
