...
%
% return range field dmod which has ndata fields
...
% 2009-JUN-18 dmod in DN such 256 DN = 1 cycle
...
%     fprintf(1,'Done building TST structure\n');
    ...
    % call looks like this:
    %   dmod            = feval(fitfun,DST,PST,TST); % return set of scalar ranges
    %   [dmod,DST]      = feval(fitfun,DST,PST,TST); % also return DST structure
...
%      DDM(1,:) * poh + DDS(1,:) * poh
    ...
    switch idatatype1
        case {0,2}  % calculate displacement: observable is phase or unwrapped or range
            if abs(volgeom1(4)) > 0
...
%     save('fitfun28.mat');return
    ...
    % call the function if volume is not zero
    switch idatatype1
       case {0,2}  % calculate displacement: observable is phase or unwrapped
             kp = 9; % 20150721
            % coseismic
...
%     disp 'u'; size u
    ...
    rng1 = rng0 + nui + bas + UCS;  % 

    % return modeled value as dmod as a column vector
    switch idatatype1
        case  0   % calculate wrapped phase, converting from meters to radians  
            dmod = 2.0 * pi * rng1 ./ DST.mpercy;
        case  -1   % calculate gradient, converting from meters/pixel to radians/pixel
            dmod = 2.0 * pi * rng1 ./ DST.mpercy;
        case 2  % calculate displacement: observable is unwrapped phase (a.k.a. range change)
            dmod = rng1;
        otherwise
...
%     fprintf(1,'Minimal value  in meters for dmod  %10.4f\n',min(dmod));
%     fprintf(1,'Maximal value  in meters for dmod  %10.4f\n',max(dmod));
