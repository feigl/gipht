%function [mhat,F,store,energy,count,msig]=anneal6(FUN,bounds,OPTIONS,varargin)
...
%Simulated annealing algorithm that tries to find a minimum to the function 'objfun'.
%
%INPUTS:
%
%'objfun' specifies the objective function.  This function should accept an input model vector
...
%    try also jacknife
% 20160524
%    change input argument list to use structures

%%Check argument syntax
if nargin<2
    help(mfilename)
    %error('Usage: [mhat,F,model,energy,count]=anneal(objfun,bounds,OPTIONS,varargin)')
...
%% unpack bounds from parameter structure
bounds(:,1) = rowvec(PST.lb);
bounds(:,2) = rowvec(PST.ub);
if size(bounds,2)~=2
...
%% unpack handle for fitting function
...
for k=1:runs
    ...
    if matrix
        %O=feval(objfun,bestmodel,varargin{:});
        PST.p1 = bestmodel;
        O=feval(objfun,DST,PST,TST);
    else
        O=zeros(100,1);
        tstart = tic;
        parfor e=1:100
            %O(e)=feval(objfun,bestmodel(:,e),varargin{:});
            PSTE(e) = PST;
            PSTE(e).p1 = bestmodel(:,e);
            O(e)=feval(objfun,DST,PSTE(e),TST);
        end
        clear PSTE;
        if talk
    ...
    for w=1:t
        ...
        for x=1:p
            if delta(x)
                ...
                count(k)=count(k)+NM;
                %                 if matrix
                %                     O=feval(objfun,modelmatrix,varargin{:});
                %                 else
                O=zeros(NM,1);
                parfor e=1:NM
                    %O(e)=feval(objfun,modelmatrix(:,e),varargin{:});
                    PSTE(e) = PST;
                    PSTE(e).p1 = modelmatrix(:,e);
                end
                parfor e=1:NM
                    O(e)=feval(objfun,DST,PSTE(e),TST);
                end
