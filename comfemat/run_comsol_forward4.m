function modeledDisplacements = run_comsol_forward4(uparams,PST,DST,TST)
% uparams == parameters with units
...
% count parameters
if numel(uparams) ~= numel(PST.name)
    error('miscount of parameters');
else
    mparams = numel(uparams);   
end

%% set parameters
nreset = 0;
for ip=1:mparams  % loop over indices to comsol parameters
    current1 = uparams(ip);  % current value 
