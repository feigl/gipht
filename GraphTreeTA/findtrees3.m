function [Qiev, Trees] = findtrees3(yyyymmdd1,yyyymmdd2)
...
edges = [colvec(yyyymmdd1),colvec(yyyymmdd2)];
...
if ncnull ~= ntrees | nrnull ~= me % size of null matrix should be equal to rank deficiency (and number of distinct components) and number of epochs
    error 'Counting problem'
else
   Trees = NaN*ones(ntrees,me);  % set up matrix for storing components
    for j=1:ntrees
        k = 0;
        for i=1:me
           if abs(N(i,j)) > 0
