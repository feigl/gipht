function h1 = plotpairs5_examp(tm,ts,Qdiff,Qdsig,Qdmod ...
...
%     end
tfitl = tfit(tfit<=tbreaks(3) & tfit >= tbreaks(2));
Qdfitl = Qdfit(tfit<=tbreaks(3) & tfit >= tbreaks(2));
tfitu = tfit(tfit>=tbreaks(3));% & tfit <= tbreaks(2));
Qdfitu = Qdfit(tfit>=tbreaks(3));% & tfit <= tbreaks(2));
tfit1 = tfit(1:2);
Qdfit1 = Qdfit(1:2);
 plot(tfit1,Qdfit1,      'k-', 'LineWidth',2); % modeled value
   plot(tfitl,Qdfitl,      'k--', 'LineWidth',2); % modeled value
   plot(tfitu,Qdfitu,      'k-', 'LineWidth',2); % modeled value
    %plot(tfit,Qfsigl,'b-.','LineWidth',1); % lower envelope 
    %plot(tfit,Qfsigu,'b-.','LineWidth',1); % upper envelope 
end
