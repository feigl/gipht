function [trees, DD, tepochs, iepochs, iuniqorbs, uniqdates] = findtrees(t0,t1,dispflag,imast,mdate,islav,sdate)
...
%2014 OCT 10 - Elena; specific to graph theory.  Gives the tree relationships (DD);
%                     row length of trees can be used to find ntrees (number of disjoint trees) 
nargchk(2,7,nargin);
...
if nargin <= 3
    ...
    islav = zeros(np,1);
    for i=1:np
        imast(i) = find(abs(tu-t0(i)) < 1/366.0);
        islav(i) = find(abs(tu-t1(i)) < 1/366.0);
    end
