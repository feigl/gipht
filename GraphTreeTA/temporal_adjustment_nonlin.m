function [pest, psig, mse, dmod, tfit, pfit, sfitl, sfitu, rd, V, G, sswr, Vx, var, res_n, best_meta] = temporal_adjustment_nonlin(data,data_sigma,tm,ts,tbreaks,tfunc,metaparams, x_int, lb, ub, A, b, Aeq, beq)
% function [pest, psig, mse, dmod, tfit, pfit, sfitl, sfitu, rd, V, G, sswr, Vx, var, res_n, best_meta] = temporal_adjustment_nonlin(data,data_sigma,tm,ts,tbreaks,tfunc,metaparams, x_int, lb, ub, A, b, Aeq, beq)
% Given time tags and data values, perform temporal adjustment with optimized values of metaparameters. Solve the weighted least squares
% problem Gm = d, where G is the design matrix, d is the data vector
% (Qdiff), and m is the unknown parameter vector. To be used with overdetermined problems. Inversion results are interpolated to find
...
%     var = sample variance
% 
% Nonlinear optimization of meatparameters is performed using fmincon along
% with a function minimizing mse of inversion.
%
% Elena Baluyut and Kurt Feigl UW-Madison
% 2015-11-09
...
%% check for correct parameterizations
if strcmp(tfunc, 'pwl') == 1 || strcmp(tfunc, 'ber')
    error('this function is for TA nonlinear optimization only (overdetermined problems).  Use temporal_adjustment.m function instead')
end
    V = incidence_to_cov(Q, dsig);
    %% Perform nonlinear optimization
% find number of metaparameters to be optimized
x_n = numel(find(metaparams == inf));

if nargin < 11 
% set constraints for less than or equal to bounds (see "fmincon" help, analogous to their A and b)
% default sets metaparameters to positive values
A = -1*ones(1, x_n);
b = 0;
% set constraints for equal to bounds (see "fmincon" for help, analogous to
% their Aeq and beq)
% default is uniformative, sets zero = zero
Aeq = zeros(1, x_n);
beq = 0; 
end
...
% constraints for bounds of x set by user (see "fmincon" for help, analogous to their lb
% and ub)
%fun = @(x) temporal_adjustment_nonlinsub(data,tm,ts,tbreaks,tfunc,metaparams, x, V);
 best_meta = fmincon(@(x) temporal_adjustment_nonlinsub(data,tm,ts,tbreaks,tfunc,metaparams, x, V), x_int, A, b, Aeq, beq, lb, ub);
%[best_meta, meta_res] = lsqcurvefit(fun, x_int, A, b, lb, ub);

metaparams(metaparams==inf) = best_meta;
...
%meta_mse = meta_res/(ndat-numel(best_meta));
...
%fprintf(1, 'best meta mse                                                                            is %20.4f\n',meta_mse);
