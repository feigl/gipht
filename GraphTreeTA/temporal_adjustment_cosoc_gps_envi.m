...
datfile = 'brick_vol_est_gps_ENVI_new.txt';
% datfile = 'brick_vol_est_gps_envi_unr.txt';
%datfile = 'brick_drho_est_gps.txt';
datfile = 'gps_pre2010_time_series_data.csv';
dataset = datfile;
...
% Qrate = (VALS(:,3));
% rs = sqrt(abs(VALS(:,4)));
% rs(find(rs > mean(rs)+5*std(rs))) = mean(rs);
% rs = rs*1e4;%*1e-4;%./(ts-tm);
% rs(find(rs > mean(rs)+5*std(rs))) = mean(rs);
...
% Qdsig(find(Qdsig > mean(Qdsig)+5*std(Qdsig))) = mean(Qdsig)+2*std(Qdsig);
...
%     [trees, DD tepochs, iepochs, iuniqorbs, uniqdates] = findtrees2(tm_dstruct',ts_dstruct');
    ...
    save('TA_gps_envi_new.mat', 'DD', 'tepochs', 'trees')
end
...
for k = 1:numel(tfuncs)
    ...
    switch tfunc
        ...
        case 'exprdecay'
            ...
            metaparams(2) = 20;%6.5;%6.5;   % characteristic time constant in years
            tbreaks = [];
...
%             tbreaks = [tu(1), dyear(2016, 03, 14), dyear(2016, 03, 19), dyear(2016, 03, 24), tu(end)]; % 
            ...
            tbreaks = [tu(1), dyear(2009, 01, 01), tu(end)];
            tbreaks = unique(sort(tbreaks));
     case {'sawtooth'};
            metaparams = nan;       
            load('Pumping/sawtooth_tbreaks.mat')
            tbreaks = sort([tu(1), dyear([2005:2009], 6*ones(size([2005:2009])), 1*ones(size([2005:2009]))), dyear([2005:2007], 12*ones(size([2005:2007])), 1*ones(size([2005:2007]))), tu(end)]);

       %                 tbreaks = sort([tu(1), dyear([2005:2009], 6*ones(size([2005:2009])), 1*ones(size([2005:2009]))), tu(end)]);
%             tbreaks = sort([tu(1), dyear([2005:2009], 12*ones(size([2005:2009])), 1*ones(size([2005:2009]))), tu(end)]);
...
[pest, psig, mse, Qdmod, tfit, pfit, sigl, sigu, pfitd, rd, V, G, data, SSWR] = temporal_adjustment_function_coso_gps_envi(Qdiff,Qdsig,tm,ts,tbreaks,tfunc,metaparams);
...
xlab = 'year'; 
    ...
    h  = plotpairs(tm,ts,Qdiff/scalefactor,Qdsig/scalefactor,Qdmod/scalefactor ...
     ...
     printeps_e(sprintf('Coso_cuboid_%d%s_envi',numel(pest), tfunc),h);    

tm_gps = tm;
ts_gps = ts;
Qdiff_gps = Qdiff/scalefactor;
Qdisg_gps = Qdsig/scalefactor;
Qdmod_gps = Qdmod/scalefactor;
tfit_gps = tfit;
pfit_gps = pfit/scalefactor;
sigl_gps = sigl/scalefactor;
sigu_gps = sigu/scalefactor;
pest_gps = pest;
tbreaks_gps = tbreaks;
