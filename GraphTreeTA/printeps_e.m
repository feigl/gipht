function printeps_e(pdffilename,ghandle)
%function printpdf(pdffilename,ghandle)
% write current graphics window to a PDF file
% last update: 20130520 Kurt

if nargin < 1
    %pdffilename = strcat(mfilename, '_', datestr(now, 'yyyy.mm.dd-HH:MM:SS'), '.eps');
    pdffilename = strcat(mfilename, '.eps');
end

% if numel(strfind(pdffilename,'.eps')) > 0
% %   pdffilename = strrep(pdffilename, '.eps', strcat('_', datestr(now, 'yyyy.mm.dd-HH:MM:SS'), '.eps'));
%     pdffilename = strrep(pdffilename, '.eps', strcat('_', datestr(now, 'yyyy.mm.dd-HH:MM:SS'), '.eps'));
% 
% end
if numel(strfind(pdffilename,'.eps')) <= 0
%     pdffilename = sprintf('%s.eps',strcat(pdffilename, '_', datestr(now, 'yyyy.mm.dd-HH:MM:SS')));
    pdffilename = sprintf('%s.eps',strcat(pdffilename));
...
if exist('ghandle','var') == 1
    print(ghandle,pdffilename,'-depsc','-r1200'); % otherwise, print PDF
else
    print(gcf,pdffilename,'-depsc','-r1200'); % otherwise, print PDF
end
