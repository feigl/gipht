%% Run geostatistical inversion on an unwrapped pair
...
%
% The geothermal field at Brady Hot Springs, Nevada has subsided over the past
% decade. Between 2004 and 2014, the rate of downward vertical displacement
% was on the order of 10 millimeters per year, as measured by two
% independent geodetic techniques: Interferometric synthetic aperture radar
% (InSAR) and Global Positioning System (GPS). The observed deformation
% field forms an approximately elliptical bowl that is 4 kilometers long
% and aligned with the trace of the NNE striking normal fault system. We
% use modeling to estimate the plausibility of pressure changes or thermal
% contraction as the cause of the observed subsidence. As a result,
% Bayesian inference favors with ?very strong evidence? thermal contraction
% over other hypotheses as the dominant driving mechanism for the observed
% subsidence. Using InSAR data spanning from 22 July 2016 and 22 August
% 2017, we estimate the volume change rate in the significantly deforming
% volume to be (-29 +/- 3) thousand cubic meters per year and the total
% rate of change in thermal energy between -53 and -79 Megawatt. We infer
% the total volume of cubes where the estimated volumetric strain rate is
% significantly different from zero with 95 percent confidence to be 119
...
%% On Dec 11, 2020, at 12:28, Reinisch, Elena Cristina <ecreinisch@lanl.gov>
% wrote: Hi Kurt, That?s exciting to hear! The HTC versions of the code
% should be available under my account on submit-3 (or is it 5 now?), and I
% think they might be available under PoroTomo?s roftp directory; however,
% it?s been so long since I?ve logged in that I don?t remember their exact
% location.  To make them easier for you to locate both now and moving
% forward, I?ve added my local copies (backups of the originals) to the
% GIPhT repo under the Brady branch.  I also had a pair-specific script for
% that paper, but I?ll need more time to find its copy on my personal
% laptop.  You might be better off working from the HTC version, though,
% because it?s not pair-specific (and I?ll admit, less messy!).  Please let
...
%     fn0 = 'drhomaskd_utm.grd';
    ...
    fn0='/System/Volumes/Data/mnt/t31/insar/ALOS/T215/brady/full/geostatistical/In20071217_20100509/x_001_OBSV_NUIS4.grd'
    INFO = grdinfo3(fn0)
